...
//     Fusb=360/8=45Mhz
//返回值:0,成功;1,失败
unsigned char Sys_Clock_Set(u32 plln,u32 pllm,u32 pllp,u32 pllq)
{
...
void Stm32_Clock_Init(u32 plln,u32 pllm,u32 pllp,u32 pllq)
{
    ...
    MY_NVIC_GROUP_CONFIG(4);
    SystemCoreClock = HSE_CLK / pllm * plln / pllp;
	//配置向量表
#ifdef  VECT_TAB_RAM
	MY_NVIC_SetVectorTable(0x20000000, 0x0);
#else
	MY_NVIC_SetVectorTable(APP_ADDR, 0x0);
#endif
}

void MY_NVIC_GROUP_CONFIG(u8 NVIC_Group)
{
...
//NVIC_Channel           :中断编号
//这里默认用的组4，所以没有对相应优先级做处理
void MY_NVIC_SET(u8 NVIC_PreemptionPriority, u8 NVIC_SubPriority, u8 NVIC_Channel)
{
    unsigned int temp;
    //获取中断分组: 0~4
    unsigned int group = (uint32_t)((SCB->AIRCR & 0x700) >> 8);
    
    if(group == 4)
    {
        if(NVIC_PreemptionPriority == 0)
            NVIC_PreemptionPriority ++;
        if(NVIC_SubPriority == 0)
            NVIC_SubPriority ++;
        temp = NVIC_PreemptionPriority * NVIC_SubPriority - 1;
    }
    else
    {
        temp = NVIC_PreemptionPriority << (4 - group);
        temp |= NVIC_SubPriority & (0xF >> group);
    }
    temp &= 0xF;//取低四位，防止错误
    //使能中断(清除中断在 NVIC->ICER 相应的位置1即可)
...
{
	unsigned char EXTOFFSET = (BITx % 4) * 4;
...
{
	unsigned int pinpos = 0, pos = 0, curpin = 0;
