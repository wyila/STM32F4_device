...
void W25QXX_Init(void)
{
    ...
    GPIOC->MODER &= ~(0x03 << (6 * 2));//清空状态
    GPIOC->MODER |= 0x01 << (6 * 2);//通用推挽输出
    
    GPIOC->OSPEEDR &= ~(0x03 << (6 * 2));//清空状态
    GPIOC->OSPEEDR |= 0x11 << (6 * 2);//高速模式 100MHz
    
    GPIOC->PUPDR &= ~(0x03 << (6 * 2));//clear status
    GPIOC->PUPDR |= 0x01 << (6 * 2);//up
    
    
	W25QXX_CS = 1;//SPI FLASH不选中
	SPI5_Init();//初始化SPI
	SPI5_SetSpeed(0);//0:fclk/2, fclk=APB2_clk,设置为45M时钟,高速模式
